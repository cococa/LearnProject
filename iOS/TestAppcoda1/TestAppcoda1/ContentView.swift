...
struct ContentView: View {
    ...
    @ObservedObject var restaurantData : RestaurantData = RestaurantData()
...
//
    ...
    var body: some View {
        NavigationView {
            List{
                ForEach(restaurantData.list.indices, id: \.self){ index in
                    NavigationLink(destination: {
                        RestaurantDetailView(restaurant: restaurantData.list[index])
                    }){
                        BasicTextImageRow(restaurant: restaurantData.list[index])
                            ...
                            .swipeActions(edge: .trailing, allowsFullSwipe: true){
                                ...
                                Button{
                                    restaurantData.list.remove(at: index)
                                }label: {
                                    Text("DELETE")
                                }
                                .tint(.red)
                ...
                .onDelete{ indexSet in
                    restaurantData.list.remove(atOffsets: indexSet)
                }
